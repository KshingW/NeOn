...
def read_img_kps(opt):
    ...
    kps_x = kps[[0, 1]].values
    kps_y = kps[[2, 3]].values
...
def run(opt):
    ...
    seg_x = F.interpolate(seg_x, size=opt['img_size'], mode='bilinear', align_corners=True)
    seg_y = F.interpolate(seg_y, size=opt['img_size'], mode='bilinear', align_corners=True)


    seg_x = torch.argmax(seg_x, dim=1, keepdim=True).float()
    seg_y = torch.argmax(seg_y, dim=1, keepdim=True).float()
    msk = ((seg_y == 3) + (seg_y == 2)).float()  # get the vessel mask
    ...
    fea_x = fea_x * disk_msk
    fea_y = fea_y * disk_msk
    print(fea_x.shape, img_x.shape)
    ...
    best_ncc = -1
    time_s = time.time()
    niter = 100 if opt['x_fp'].split('/')[-1].split('_')[0] != 'A01' else 300
    print(niter)
    for idx in range(niter):
        ...
        if eval_tre < best_tre:
            ...
            # np.save(f'save_field_30/{sub_id}_best_field_tre.npy', pos_flow)
        if ncc_score > best_ncc:
            best_ncc = ncc_score
            best_ncc_iter = idx
            sub_id = opt['x_fp'].split('/')[-1].split('_')[0]
            # np.save(f'save_field_30/{sub_id}_best_field.npy', pos_flow)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ori_size", type = str, default = '(2912,2912)')
    ...
    pre_align_FIRE = '/scratch/dl3837/neuralforce/neon_oct/ipa_cnn'
    pre_align_img = '/scratch/dl3837/neuralforce/neon_oct/ipa_org_img'
    pre_align_txt = '/scratch/dl3837/neuralforce/neon_oct/ipa_txt'
    sub_list = [i.split('_')[0] for i in os.listdir(pre_align_FIRE)]
    ...
    for sub in sub_list:
        ...
        sub_id = sub
        opt.update({
            'fea_x_fp': f'{pre_align_FIRE}/{sub_id}_1_feature.pt',
            'fea_y_fp': f'{pre_align_FIRE}/{sub_id}_2_feature.pt',
            'seg_x_fp': f'{pre_align_FIRE}/{sub_id}_1_logit.pt',
            'seg_y_fp': f'{pre_align_FIRE}/{sub_id}_2_logit.pt',
            'x_fp': f'{pre_align_img}/{sub_id}_1.jpg',
            'y_fp': f'{pre_align_img}/{sub_id}_2.jpg',
            'kps_fp': f'{pre_align_txt}/control_points_{sub_id}_1_2.txt',
        })
    ...
    import csv
    with open('neon_fire.csv', 'w', newline='') as f:
        writer = csv.writer(f)
